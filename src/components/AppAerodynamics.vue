...
export default {
  ...
  async mounted() {
    this.loading = true;
    console.log('[AppAerodynamics] - Mounted - Попытка загрузки контента');
    try {
      this.content = await this.$store.dispatch('content/fetchContent', {
        contentType: 'landingContent',
        componentKey: 'aerodynamics'
      });
      console.log('[AppAerodynamics] - Mounted - Контент загружен:', this.content);
    } catch (error) {
